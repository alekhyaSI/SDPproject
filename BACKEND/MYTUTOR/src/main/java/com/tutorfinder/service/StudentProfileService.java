 ...
 // Get all student profiles
    public List<StudentProfileDTO> getAllProfiles() {
        return repository.findAll().stream()
                .map(profile -> {
                    StudentProfileDTO dto = new StudentProfileDTO();
                    dto.setUserId(profile.getUserId()); // set ID
                    dto.setName(profile.getName());
                    dto.setEmail(profile.getEmail());
                    dto.setPhone(profile.getPhone());
                    dto.setLocation(profile.getLocation());
                    ...
                    try {
                        List<StudentProfileDTO.SubjectAvailabilityDTO> subjects =
                                profile.getSubjectsAvailabilityJson() != null
                                        ? objectMapper.readValue(profile.getSubjectsAvailabilityJson(),
                                            new com.fasterxml.jackson.core.type.TypeReference<List<StudentProfileDTO.SubjectAvailabilityDTO>>() {})
                                        : List.of();
                        dto.setSubjectsAvailability(subjects);
                    } catch (Exception e) {
                        dto.setSubjectsAvailability(List.of());
                    }
